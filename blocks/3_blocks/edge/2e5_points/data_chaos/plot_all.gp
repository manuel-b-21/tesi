...
set fit quiet
unif_min=61 #(m-1)*L=unif_min
unif_max=101 #(m-1)*L=unif_max
#unif_min=96
#unif_max=108.5
...
set label at graph 0.55,0.15 "N_{samples}=2×10^5" font ",10" front
...
unset label
set label at graph -0.127,1.05 "{/:Bold d}" font ",10"
set bars small
set xlabel "w = (m-1)LT (ms)"
...
set yrange [0:2]
f(x) = mle_mean
fit [*:*][0:1500] f(x) "mle.dat" using (($1-1)*$2*T) : \
